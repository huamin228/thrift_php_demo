...
class TSocket extends TTransport {
  ...
  public function open() {
    ...
    if ($this->port_ <= 0) {
      ...
      $server_string = 'tcp://'.$this->host_.':'.$this->port_;
      $flag =  $this->persist_ ?  STREAM_CLIENT_CONNECT | STREAM_CLIENT_PERSISTENT :  STREAM_CLIENT_CONNECT;
      $this->handle_ = @stream_socket_client($server_string, $errno, $errstr, $this->sendTimeoutSec_ + ($this->sendTimeoutUsec_ / 1000000), $flag);
...
//    if (!$this->persist_) {
      log_message('debug', 'TSocket Close');
      @fclose($this->handle_);
      $this->handle_ = null;
//    }
  }
